...
static int en8811h_phy_probe(struct phy_device *phydev)
{
    ...
    struct device *dev = phydev_dev(phydev);
    bool pnswap_rx = false;
    bool pnswap_tx = false;
    ...
    if (phydev->mdio.dev.of_node) {
        pnswap_rx = of_property_read_bool(phydev->mdio.dev.of_node, "airoha,pnswap-rx");
        pnswap_tx = of_property_read_bool(phydev->mdio.dev.of_node, "airoha,pnswap-tx");
    }
    ...
    if (pnswap_rx)
        pbus_value |= EN8811H_RX_POLARITY_REVERSE;
    else
        pbus_value |= EN8811H_RX_POLARITY_NORMAL;

    if (pnswap_tx)
        pbus_value |= EN8811H_TX_POLARITY_REVERSE;
    else
        pbus_value |= EN8811H_TX_POLARITY_NORMAL;
